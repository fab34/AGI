---
name: agi
description: 當使用者要為新客戶規劃或撰寫全新的 Brand Guideline、品牌手冊、視覺識別規範時使用。互動教練模式：一次只問一題完成 intake，全部問完後產出由知識圖譜推理驅動的 Plan 供確認，使用者確認後才 Build 並寫入 clients/ 下的 blueprint、index.yaml 與 note.md 初稿。Plan 階段以 Design Strategy 推理鏈（產業 → 品牌類型 → 商業問題 → 匹配決策卡 → 架構）運作，並引用 references/research/decision_patterns.yaml、contexts、modules、archetypes、module_graph 與 references/docs/*.profile.yaml 的證據。本 skill 亦涵蓋兩個延伸能力（見 references/）：分析既有 guideline PDF（design-guideline-researcher）、把分析升級成決策卡（design-strategy-extractor）。
---
