---
name: agi
description: 當使用者要為新客戶規劃或撰寫全新的 Brand Guideline、品牌手冊、視覺識別規範時使用。互動教練模式：一次只問一題完成 intake，全部問完後產出由知識圖譜推理驅動的 Plan 供確認，使用者確認後才 Build 並寫入 assets/clients/ 下的 blueprint、index.yaml 與 note.md 初稿。Plan 階段以 Design Strategy 推理鏈（產業 → 品牌類型 → 商業問題 → 匹配決策卡 → 架構）運作，並引用 references/research/decision_patterns.yaml、contexts、modules、archetypes、module_graph 與 references/docs/*.profile.yaml 的證據。本 skill 亦涵蓋兩個延伸能力（見 references/）：分析既有 guideline PDF（design-guideline-researcher）、把分析升級成決策卡（design-strategy-extractor）。
---
...
1. **執行前必讀本檔**：以 repo 內 `.agents/skills/agi/SKILL.md` 為準；若對話中附帶的 skill 摘要與本檔衝突，**一律以本檔為準**。
...
欄位定義見 [`references/research/intake_schema.yaml`](references/research/intake_schema.yaml)。`compliance_needs` 若使用者回答「無」可記為 `[none]`。
...
- `industry`：對照 [`references/research/contexts/industries.yaml`](references/research/contexts/industries.yaml) 的 id 與 aliases（如 `new_energy_mobility`、`finance`、`luxury_premium`）。
- `brand_type`：對照 [`references/research/contexts/brand_types.yaml`](references/research/contexts/brand_types.yaml)（如 `challenger`、`category_creator`、`institutional_public_trust`）。
...
讀取知識庫資源後，在 **單一則回覆** 中輸出完整 Plan（使用下方模板）。同時可寫入 `assets/clients/<client-slug>/plan.md`，frontmatter 設 `status: pending_confirmation`。

### 知識庫資源（規劃 Plan 時必讀）

1. [`references/research/decision_patterns.yaml`](references/research/decision_patterns.yaml) — **L3 決策圖譜**（含 by_industry / by_brand_type / by_module / by_signal 反向索引）
2. [`references/research/contexts/industries.yaml`](references/research/contexts/industries.yaml)、[`brand_types.yaml`](references/research/contexts/brand_types.yaml) — **L4 情境知識**
3. [`references/research/modules/`](references/research/modules/) — **L2 固定知識**（含 `driven_by` 決策驅動映射）
4. [`references/research/archetypes.yaml`](references/research/archetypes.yaml)、[`references/research/module_graph.yaml`](references/research/module_graph.yaml)、[`references/research/intake_schema.yaml`](references/research/intake_schema.yaml)
5. 相關 `references/docs/*.profile.yaml` 與先例 `note.md`（1–3 份）
...
客戶訊號（industry / brand_type / business_problem_signals）
  ...
  → L5 選 archetype（references/research/archetypes.yaml）＋ L2 決定固定模組（references/research/modules/）
...
- Typography：因客戶人格「…」→ 傾向「…」（依 references/research/modules/typography-system.yaml）
- Color：因客戶定位「…」→ 傾向「…」
...
`assets/clients/<slug>/` 下的 intake.yaml、blueprint.yaml、index.yaml、note.md、coach_session.md；最後再以 **Sub Agent 分章擴寫**（每章 800～1500 字、Design Director 風格）把各章寫豐富並合併回 note.md
...
3. **避免寫檔衝突**：每個 Sub Agent **只寫自己負責的章節檔** `assets/clients/<slug>/sections/<chapter-id>.md`，**不得**同時編輯 `<slug>.note.md`。
...
- **先例參考**：相關 `precedent_id` 對應的 `references/docs/*.note.md` 章節（借鏡結構與決策邏輯，**不抄** HEX／標語／尺寸）
...
- **輸出位置**：寫入 `assets/clients/<slug>/sections/<chapter-id>.md`，回報完成的章節與字數
...
- `index.yaml` schema 同 researcher 模式（見 `references/design-guideline-researcher.md`）
...
1. 先用 `references/research/decision_patterns.yaml` 的反向索引（by_industry / by_brand_type / by_signal）取命中決策卡，再取其 `evidence.brand_id` 作為第一手先例
2. `references/docs/*.profile.yaml` 依 archetype 篩選、比對 modules 重疊
...
- 「繼續 assets/clients/acme-corp」→ 讀既有檔案：若只有 `plan.md` 且 pending，繼續等確認；若已 build，可補章節
- 使用者未確認 Plan 就要求寫稿 → 提醒先確認 Plan

若 `assets/clients/<slug>/` 已存在且 `blueprint.plan_status: confirmed`，Build 已完成；補寫章節時不要覆蓋已確認內容除非使用者要求。
