...
```text
sources/                          # 原始 PDF（repo 根目錄，gitignore）
  Example_Guideline.pdf
references/docs/
  Example_Guideline.index.yaml   # 結構層：章節樹
  Example_Guideline.note.md      # 研究層：十一維分析
  Example_Guideline.profile.yaml # 聚合層：跨語料可查詢欄位（可選，見 .agents/skills/agi/scripts/generate_profiles.py）
```

如果使用者提供不同路徑，使用使用者指定的路徑；否則預設：

- 來源 PDF 放在 `sources/`（repo 根目錄）。
- 輸出檔案放在 `references/docs/`。
- 每份 PDF 對應到 `references/docs/<PDF 檔名不含副檔名>.index.yaml` 與 `references/docs/<PDF 檔名不含副檔名>.note.md`。
- 聚合層 `references/docs/<PDF-name>.profile.yaml` 由 `python .agents/skills/agi/scripts/generate_profiles.py` 從 index + note 產生，供跨品牌統計與 Agent 檢索；schema 見 `assets/schemas/profile.schema.yaml`。

## 增量處理

在讀取或重新處理 PDF 前，先檢查目標輸出是否已存在：

- `references/docs/<PDF-name>.index.yaml`
- `references/docs/<PDF-name>.note.md`
...
1. **尋找來源 PDF**
   ...
   - 將每個 `file.pdf` 對應到 `references/docs/file.index.yaml` 與 `references/docs/file.note.md`。
...
- 每份已處理 PDF 都有 `references/docs/<PDF-name>.index.yaml`。
- 每份已處理 PDF 都有 `references/docs/<PDF-name>.note.md`。
