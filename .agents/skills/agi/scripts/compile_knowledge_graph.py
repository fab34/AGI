...
讀取 references/docs/*.decisions.yaml（各文件 sidecar），執行：
  ...
  4. 輸出全域圖譜 references/research/decision_patterns.yaml（含 by_industry /
     by_brand_type / by_module / by_signal 反向索引）。
  5. 交叉檢查 references/research/contexts/*.yaml 與 references/research/modules/*.yaml 中引用的
     dp- id 是否已萃取，列出待補（backlog）。
  6. 輸出報告 references/research/knowledge_graph_report.md。

用法：python .agents/skills/agi/scripts/compile_knowledge_graph.py
依賴：PyYAML
"""
...
def main() -> None:
    ...
    graph = {
        ...
        "source": "references/docs/*.decisions.yaml",
        "stats": {
    ...
    header = (
        "# AGI 決策圖譜（L3）— 由 .agents/skills/agi/scripts/compile_knowledge_graph.py 自動產生\n"
        "# 請勿手動編輯；新增決策卡請改 references/docs/<brand_id>.decisions.yaml 後重跑。\n"
        "# schema：assets/schemas/decision_pattern.schema.yaml\n\n"
    )
    ...
    lines = [
        ...
        f"產生命令：`python .agents/skills/agi/scripts/compile_knowledge_graph.py`",
        "",
        "## 摘要",
        "",
    ...
    if invalid_evidence:
        lines += [
            ...
            "",
            "下列 evidence 的 brand_id 或 chapter 在 references/docs/*.index.yaml 找不到，需修正：",
            "",
    ...
    if unknown_evidence_brands:
        lines += [
            ...
            "",
            "下列 evidence_brands 在 references/docs/ 沒有對應 index.yaml（可能拼字或大小寫不符）：",
            "",
            "| 檔案 | entry | brand_id |",
            "|------|-------|----------|",
        ]
