#!/usr/bin/env python3
"""從 references/docs/*.profile.yaml 產生 references/research/corpus_report.md"""
...
OUT = ROOT / "references" / "research" / "corpus_report.md"
...
def generate_report(profiles: list[dict]) -> str:
    ...
    lines: list[str] = [
        ...
        f"資料來源：`references/docs/*.profile.yaml`",
        f"產生命令：`python .agents/skills/agi/scripts/generate_corpus_report.py`",
        "",
        "## 摘要",
        "",
    ...
    lines.extend([
        "",
        "定義見 `references/research/archetypes.yaml`；分類：`python .agents/skills/agi/scripts/assign_archetypes.py`。",
        "",
        "---",
        "",
        "## 1. 模組出現頻率（文件級）",
    ...
    lines.extend([
        ...
        "## 8. 對第二階段的建議",
        "",
        "1. **Archetype 定義**：已完成 `references/research/archetypes.yaml`；profile 已回填 archetype。",
        "2. **Module graph**：§1 模組共現 + note 的 Relationship 可合併為 `references/research/module_graph.yaml`。",
